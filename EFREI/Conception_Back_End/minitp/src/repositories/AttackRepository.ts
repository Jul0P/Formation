...
class AttackRepository {
  ...
  public async findById(id: number): Promise<Attack | null> {
    const row = await database.getOne('SELECT * FROM attack WHERE id = $1', [id]);
    if (!row) {
      return null;
    }
  ...
  public async findAll(): Promise<Attack[]> {
    const result = await database.query('SELECT * FROM attack');
  ...
  public async create(attack: Attack): Promise<Attack> {
    const result = await database.getOne('INSERT INTO attack (name, damage, usage_limit) VALUES ($1, $2, $3) RETURNING *', [
      attack.name,
      attack.damage,
      attack.usageLimit,
    ]);
  ...
  public async update(attack: Attack): Promise<void> {
    await database.query('UPDATE attack SET name = $1, damage = $2, usage_limit = $3 WHERE id = $4', [
      attack.name,
      attack.damage,
      attack.usageLimit,
      attack.id,
  ...
  public async delete(id: number): Promise<void> {
    await database.query('DELETE FROM attack WHERE id = $1', [id]);
  }
}
