...
class TrainerRepository {
  ...
  public async findById(id: number): Promise<Trainer | null> {
    const row = await this.database.getOne('SELECT * FROM trainer WHERE id = $1', [id]);
    if (!row) {
      return null;
    }
  ...
  public async findAll(): Promise<Trainer[]> {
    const result = await this.database.query('SELECT * FROM trainer');
  ...
  public async create(trainer: Trainer): Promise<Trainer> {
    const result = await this.database.getOne('INSERT INTO trainer (name, level, experience) VALUES ($1, $2, $3) RETURNING *', [
      trainer.name,
      trainer.level,
      trainer.experience,
    ]);
  ...
  public async update(trainer: Trainer): Promise<void> {
    await this.database.query('UPDATE trainer SET name = $1, level = $2, experience = $3 WHERE id = $4', [
      trainer.name,
      trainer.level,
      trainer.experience,
      trainer.id,
  ...
  public async delete(id: number): Promise<void> {
    await this.database.query('DELETE FROM trainer WHERE id = $1', [id]);
  }
}
